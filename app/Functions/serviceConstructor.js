...
module.exports = function Service(CurrentMoment,service_id,service_date,service_description,linked_unit,second_unit,second_unit_lat,second_unit_long,speed,compass,location_age,schedule_variance,lat,long){
  ...
  this.location_age_seconds = parseInt(this.location_age.toString().split(":")[0]*60) + parseInt(this.location_age.toString().split(":")[1]);
  ...
  //generate Status Messages (used to be own method, but needed too many variables)
      ...
      if(this.location_age_seconds >=180 && this.kiwirail == false){
        ...
        if(this.direction == "UP" && this.laststation == "MAYM" && (this.location_age_seconds < 900) ){
          ...
          var first = {latitude: this.lat,
                     longitude: this.long,}
          var sec = {latitude: this.second_unit_lat,
