...
module.exports = function Service(CurrentMoment,
  ...
  // used to be own function, but needed too many variables
      ...
      if (this.location_age_seconds >=180 && this.kiwirail == false) {
        ...
        let tunnelExceptionsList = [
          ...
          {tunnelName: 'Tunnel 1', line: 'KPL', statusMessage: 'In Tawa Tunnel', southStation: 'KAIW', northStation: 'T2', secondsTheshold: 600},
          {tunnelName: 'Tunnel 2', line: 'KPL', statusMessage: 'In Tawa Tunnel', southStation: 'T1', northStation: 'TAKA', secondsTheshold: 240},
        ];
        // identify tunnel tracking issues
        if (this.direction == 'UP') {
          for (tunnel of tunnelExceptionsList) {
            if (tunnel.southStation == this.laststation && tunnel.secondsTheshold > this.location_age_seconds && tunnel.line == this.line) {
          ...
          for (tunnel of tunnelExceptionsList) {
            if (tunnel.northStation == this.laststation && tunnel.secondsTheshold > this.location_age_seconds && tunnel.line == this.line) {
              TempStatus = tunnel.statusMessage;
              StatusArray[1] = TempStatus;
            };
          };
  ...
  function getCrewDetails(serviceId, currentRosterDuties) {
      ...
      let serviceRosterItems = currentRosterDuties.filter((currentRosterDuties) => currentRosterDuties.dutyName == serviceId);
          ...
          for (d = dutyIndex+1; d < staffRosterItems.length; d++) {
            if (staffRosterItems[d].dutyType.substring(0, 4) == 'TRIP') {
              this.nextService = {
                ...
                serviceDeparts: getTimetableDetails(staffRosterItems[d].dutyName, '', false).departs,
                serviceDepartsString: moment(getTimetableDetails(staffRosterItems[d].dutyName, '', false).departs).format('HH:mm'),
                turnaround: getTurnaroundFrom2Times(getTimetableDetails(serviceId, '', false).arrives, getTimetableDetails(staffRosterItems[d].dutyName, '', false).departs),
              };
              break;
            };
  ...
  */
  ...
  function getSequenceStnDetails(trainMeterage, direction, serviceId, nextOrPrev) {
    ...
    for (st = 0; st < currentTimetable.length; st++) {
      if ((direction == 'UP' && nextOrPrev == 'next') || (direction == 'DOWN' && nextOrPrev == 'prev')) {
          if (currentTimetable[st].serviceId == serviceId && getMeterageOfStation(currentTimetable[st].station) > meterage) {
      ...
      } else if ((direction == 'DOWN' && nextOrPrev == 'next') || (direction == 'UP' && nextOrPrev == 'prev')) {
          if (currentTimetable[st].serviceId == serviceId && getMeterageOfStation(currentTimetable[st].station) < meterage) {
              station = currentTimetable[st].station;
              time = currentTimetable[st].departs;
              stationMeterage = getMeterageOfStation(currentTimetable[st].station);
   ...
   */
  function getScheduleVariance(kiwirailBoolean, currentTime, meterage, prevStationTime, nextStationTime, prevStationMeterage, nextStationMeterage, locationAgeSeconds) {
    // the ignore criteria
